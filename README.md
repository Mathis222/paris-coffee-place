...
- **Distance au métro + concurrents** : `src/features/localisation.py`. Coordonnées cafés = `coordonneeLambertAbscisseEtablissement/Ordonnee` du fichier Sirene actuel (déjà en Lambert93, pas besoin de géocoder pour les données d'entraînement — le géocodage BAN est réservé à l'adresse saisie par l'utilisateur final). Piège rencontré : la colonne contient parfois la valeur littérale `"[ND]"` (non déterminé) en plus des vrais `NaN` — nécessite `pd.to_numeric(..., errors='coerce')` avant tout calcul, sinon `min()/max()` sur une colonne texte donne un résultat absurde. ~1116 cafés sans coordonnées exploitables, exclus des features spatiales.
  ...
  - Sauvegarde : `data/processed/siren-neighbors-processed.parquet`.
- **POI touristiques (OSM)** : `src/ingestion/osm.py`, via `osmnx.features_from_place()` sur "Paris, France", tags `tourism` (hôtel, musée, attraction, œuvre d'art, point de vue) + `historic` (monument, mémorial, château). 6142 POI trouvés. Géométries mixtes (points, polygones, lignes) → toujours converties en un point unique via `.centroid` (fonctionne uniformément quel que soit le type). Reprojection Lambert93 **avant** le calcul du centroïde (plus correct géométriquement qu'après, en système métrique plutôt que WGS84). Sauvegarde : `data/processed/tourism-processed.parquet`.
  - Décision motivée par une remarque terrain : la population résidente (Filosofi, pas encore intégrée) ne reflète pas la fréquentation réelle du centre de Paris, largement touristique/professionnelle plutôt que résidentielle — les POI touristiques sont un signal plus direct que la densité d'habitants pour ces quartiers, d'où la priorisation avant Filosofi.
  - Features ajoutées à `localisation.py` : `dist_tourism_proche`, `nb_tourism_200`, `nb_tourism_500` (même pattern `KDTree` que le métro/concurrents, un seul arbre `tree_tourism` réutilisé pour `.query()` et `.query_ball_point()`).
  - Bug rencontré et corrigé : `query_ball_point` doit être interrogé avec les coordonnées **cafés**, jamais avec les coordonnées de la source elle-même (piège rencontré deux fois : une fois avec `tourism_coords` au lieu de `cafe_coords`, une fois avec `tree_station` réutilisé par erreur à la place de `tree_cafe` pour le comptage des concurrents — dans les deux cas ça change silencieusement ce qui est compté).
- **Pas commencé** : zones piétonnes (Paris Data), compteurs vélo/routiers (proxy rythme horaire), INSEE Filosofi (population/revenu par carreau 200m), échantillon de points négatifs/contrôle.
